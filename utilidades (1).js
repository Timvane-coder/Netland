        ...
        switch(comandoSemPrefixo){ 
            ...
            case 'linking':
                try{
                    ...
                    await socket.responderTexto(c, id_chat, criarTexto(comandos_info.utilidades.linking.msgs.resposta, resultadoLink), mensagem)
                } catch(err){
                    if(!err.erro) throw err
                    await socket.responderTexto(c, id_chat, criarTexto(comandos_info.outros.erro_api, comando, err.erro) , mensagem)
                }
            ...
            case "criarimg":
                try{
                    ...
                    await socket.responderTexto(c, id_chat, "downloading image...", mensagem);
            ...
            case "lyrics":
                try{
                    ...
                    await socket.responderArquivoLocal(c, tiposMensagem.imagem, id_chat, resultadoLetra.imagem, criarTexto(comandos_info.utilidades.lyrics.msgs.resposta, resultadoLetra.titulo, resultadoLetra.artista, resultadoLetra.letra), mensagem)
                } catch(err){
                    if(!err.erro) throw err
                    await socket.responderTexto(c, id_chat, criarTexto(comandos_info.outros.erro_api, comando, err.erro) , mensagem)
                }
            ...
            case "pro":
                try{
                    if(!args.length) return await socket.responderTexto(c, id_chat,erroComandoMsg(comando, botInfo),  mensagem);
                    let usuarioTexto = texto_recebido
                    let produtoId = parseInt(usuarioTexto);
                    let { resultado: resultadoProduto } = await api.Gerais.obterDetalhesProduto(produtoId);
                    await socket.responderArquivoLocal(c,tiposMensagem.imagem,resultadoProduto.imagem, criarTexto(comandos_info.utilidades.pro.msgs.resposta,resultadoProduto.titulo, resultadoProduto.preco, resultadoProduto.categoria, resultadoProduto.descricao),mensagem);
                } catch(err){
                    if(!err.erro) throw err
                    await socket.responderTexto(c, id_chat,criarTexto(comandos_info.outros.erro_api,comando, err.erro),mensagem);
                }
